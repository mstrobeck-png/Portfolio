...
void MyCamera::SetPositionTargetAndUpward(vector3 a_v3Position, vector3 a_v3Target, vector3 a_v3Upward)
{
	...
	//instead of super, just initalize values that super was passing
...
void MyCamera::MoveForward(float a_fDistance)
{
	...
	//changed to -a_fDistance for a proper "move forward and backwards" feel
	// 
	// Turns out this needed to be changed in order to work based on rubric; camera space vs world space
	// 
	//m_v3Position += vector3(0.0f, 0.0f, -a_fDistance);
	//m_v3Target += vector3(0.0f, 0.0f, -a_fDistance);


	//forward contributes to w & s; back and forward
...
void MyCamera::MoveSideways(float a_fDistance)
{
	...
	//if -= then the controls would be opposite
	//m_v3Position += vector3(a_fDistance, 0.0f, 0.0f);
	//m_v3Target	 += vector3(a_fDistance, 0.0f, 0.0f);


	//the rightward should make value move sideways based on information in MyCamera.h
	//contributes to a & d; left and right
...
void MyCamera::CalculateView()
{
	...
	//m_v3Upward = vector3(m_v3PitchYawRoll, 0.0f, 0.0f);

	//m_m4View affects camera placement; dont touch

	//use quaternions based on notes, ideally two functions, based on PitchYaw
	
	//started out with just m_v3PitchYawRoll and attempted to use ChangeYaw and ChangePitch only
	//to see that it would not initalize the attempt with using angleAxis

	//Create the pitch(upward) & yaw(rightward) quaternion that will change 
	quaternion q_Pitch;
	quaternion q_Yaw;

	//set the left and right angle of the camera to change, must obviously use x value,
	//also needs to include rightward
	//angleAxis - use with quaternion when creating angles and normalizeing the axis
	//axis and normalizing?
	//https://glm.g-truc.net/0.9.4/api/a00131.html glm website with different functions glm utlizes. 
	q_Pitch = glm::angleAxis(m_v3PitchYawRoll.x, m_v3Rightward);

	//normalizing is a no; may calulate in same direction as original but broke program
	//q_Pitch = glm::normalize(m_v3PitchYawRoll.x, m_v3Rightward)
	

	//sets up and down; needs PitchYawRoll y value
	//must use upward because forward is back and forth
	//angleAxis is also used here ^^^ same reason as above.

	//https://glm.g-truc.net/0.9.4/api/a00131.html glm website with different functions glm utlizes.
	//normalizing did not work
	//q_Yaw = glm::normalize(m_v3PitchYawRoll.y, m_v3Upward)
	q_Yaw = glm::angleAxis(m_v3PitchYawRoll.y, m_v3Upward);


	//I set the speed at which the camera moves to avoid headaches
	// (I have very bad motion sickness so you can comment the two next lines out
	// for grading if you have to; I just needed it to prevent throwing up)
	float speedControl = 0.10f;
	//multiply default pitchyaw(0.001f) degree value by value to decrease movement speed
	m_v3PitchYawRoll *= speedControl;

	//m_v3PitchYawRoll.x *= 0.10f;
	//m_v3PitchYawRoll.y *= 0.10f;

	//now set the up, down, left, right with mouse click
	//rotates to look at specific angles
	m_v3Forward = glm::rotate((q_Pitch + q_Yaw),m_v3Forward);
	//cross - the cross product of two values aka the forward and upward
	//tried dot product but it was not pretty.
	//m_v3Rightward = glm::dot(m_v3Forward, m_v3Upward);
	m_v3Rightward = glm::cross(m_v3Forward, m_v3Upward);

	
	//now that rotate and cross is done, set the target
	//target needs to be facing forward initally in starting postion so
	// forward + position = target
	m_v3Target = m_v3Forward + m_v3Position;
	//m_v3Target = m_v3Upward + m_v3Position;

	//postion, target, and above; what is above the camera is the only remaining factor
	//to set. Therefore above = to something. Since setting position, it is above = to position and another
	//value that is up
	m_v3Above = m_v3Upward + m_v3Position;
	//since upward is the only remaining value that works with above, it is by default
	//the best answer as far as MyCamera.h shows

	//I did change the ChangePitch, ChangeYaw, ChangeRoll to prevent the camera from locking up
	//by adding cannot exceed 90 degrees and that seems to make it run nicer
